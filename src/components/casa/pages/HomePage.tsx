  ...
  return (
    <>
      <section className="relative isolate overflow-hidden">
        ...
        <div ref={heroRef} className={`reveal mx-auto flex min-h-[88vh] max-w-7xl flex-col justify-end px-5 pb-16 pt-32 md:px-8 md:pb-24 md:pt-40 ${isRtl ? "text-right" : ""}`}>
          ...
          <h1 className="max-w-4xl font-serif text-5xl leading-[1.05] tracking-tight text-foreground md:text-7xl lg:text-8xl">
            {tt.home.heroTitle1} <span className="italic text-primary">{tt.home.heroTitleAccent}</span><br className="hidden md:block" /> {tt.home.heroTitle2}
          </h1>
          <p className="mt-6 max-w-xl text-base text-muted-foreground md:text-lg">{tt.home.heroLead}</p>
      ...
      <Section lang={lang} eyebrow={tt.home.featuredEyebrow} title={tt.home.featuredTitle} intro={tt.home.featuredIntro}>
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {featured.map((s) => <ServiceCard key={s.id} service={s} lang={lang} />)}
        </div>
        <div className="mt-10">
      ...
      <div className="border-y border-border/60 bg-card/40">
        <Section lang={lang} eyebrow={tt.home.productsEyebrow} title={tt.home.productsTitle} intro={tt.home.productsIntro} className="py-20 md:py-24">
          <div className="grid gap-6 md:grid-cols-3">
            {productPreview.map((p) => <ProductCard key={p.id} product={p} lang={lang} />)}
          </div>
          <div className="mt-10">
            <Link to={localePath(lang, "/products")} className="text-sm text-primary hover:underline">
              {tt.cta.shopAll}
            </Link>
          </div>
        </Section>
        ...
        <div className="grid items-center gap-12 md:grid-cols-2">
          <div>
            ...
            <ul className="mt-8 space-y-4 text-sm">
              <li className="flex items-center gap-3"><Calendar className="h-4 w-4 text-primary" /> {tt.home.bookFeatures[0]}</li>
              <li className="flex items-center gap-3"><Clock className="h-4 w-4 text-primary" /> {tt.home.bookFeatures[1]}</li>
              <li className="flex items-center gap-3"><MapPin className="h-4 w-4 text-primary" /> {tt.home.bookFeatures[2]}</li>
            </ul>
            <Link
              to={localePath(lang, "/reservation")}
          ...
          <div className="rounded-lg border border-border/60 bg-card p-6 shadow-elegant">
              ...
              {["10:00", "11:30", "13:00", "14:30", "16:00", "17:30", "19:00", "20:30"].map((time, i) => (
                <div
                  ...
                  className={`rounded-md border py-3 transition-colors ${
                    i === 3 ? "border-primary bg-primary text-primary-foreground" : "border-border/60 text-muted-foreground hover:border-primary/60 hover:text-foreground"
                  }`}
                >
                  {time}
                </div>
            ...
            <div className="mt-5 border-t border-border/60 pt-5 text-xs text-muted-foreground">{tt.home.liveAvailability}</div>
          </div>
        </div>
      </Section>
        ...
        <Section lang={lang} eyebrow={tt.home.appEyebrow} title={tt.home.appTitle}>
          <div className="grid items-center gap-10 md:grid-cols-2">
            ...
            <div className="relative mx-auto aspect-[9/16] w-56 overflow-hidden rounded-[2rem] border-4 border-foreground/10 bg-background shadow-elegant md:w-64">
              <img src={placeholder} alt={tt.home.appCardTitle} loading="lazy" className="h-full w-full object-cover opacity-80" />
              <div className="absolute inset-0 bg-gradient-to-b from-transparent via-background/30 to-background/90" />
              <div className="absolute inset-x-0 bottom-0 p-5">
                <div className="label-eyebrow mb-1">{lang === "ar" ? "تطبيق كازا" : "Casa App"}</div>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {tt.home.testimonials.map((tm) => (
            <figure key={tm.name} className="rounded-lg border border-border/60 bg-card p-7">
              ...
              <blockquote className="mt-5 font-serif text-2xl leading-snug text-foreground">"{tm.quote}"</blockquote>
          ...
          <div className="grid gap-10 md:grid-cols-2">
            <div ref={ctaRef} className="reveal">
              <ul className="space-y-5 text-base">
                <li className="flex items-start gap-3"><MapPin className="mt-1 h-5 w-5 text-primary shrink-0" /> <span>{lang === "ar" ? CASA.addressAr : CASA.address}</span></li>
                <li className="flex items-start gap-3"><Clock className="mt-1 h-5 w-5 text-primary shrink-0" /> <span>{lang === "ar" ? CASA.hoursAr : CASA.hours}</span></li>
              </ul>
            ...
            <div className="aspect-[4/3] overflow-hidden rounded-lg border border-border/60 bg-card">
              <img src={placeholder} alt={tt.home.visitTitle} loading="lazy" className="h-full w-full object-cover opacity-80" />
            </div>
          </div>
        </Section>
      </div>
