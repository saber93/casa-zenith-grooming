...
BEGIN
  ...
  IF v_seed_services THEN
    ...
    ELSIF v_type IN ('salon', 'ladies_salon') THEN
      v_seed := jsonb_build_array(
        jsonb_build_object('title_en', 'Hair Styling', 'title_ar', 'تصفيف الشعر', 'slug', 'hair-styling', 'price', 180, 'duration', 45),
        jsonb_build_object('title_en', 'Blow Dry', 'title_ar', 'سشوار', 'slug', 'blow-dry', 'price', 140, 'duration', 40),
        jsonb_build_object('title_en', 'Hair Coloring', 'title_ar', 'صبغ الشعر', 'slug', 'hair-coloring', 'price', 300, 'duration', 90),
    ...
    ELSIF v_type = 'massage' THEN
      v_seed := jsonb_build_array(
        jsonb_build_object('title_en', 'Swedish Massage', 'title_ar', 'مساج سويدي', 'slug', 'swedish-massage', 'price', 260, 'duration', 60),
        jsonb_build_object('title_en', 'Deep Tissue Massage', 'title_ar', 'مساج الأنسجة العميقة', 'slug', 'deep-tissue-massage', 'price', 320, 'duration', 75),
        jsonb_build_object('title_en', 'Foot Massage', 'title_ar', 'مساج القدمين', 'slug', 'foot-massage', 'price', 160, 'duration', 45),
        jsonb_build_object('title_en', 'Relaxation Massage', 'title_ar', 'مساج استرخاء', 'slug', 'relaxation-massage', 'price', 240, 'duration', 60)
      );
    ELSIF v_type = 'beauty_salon' THEN
      v_seed := jsonb_build_array(
        jsonb_build_object('title_en', 'Hair Styling', 'title_ar', 'تصفيف الشعر', 'slug', 'hair-styling', 'price', 180, 'duration', 45),
        jsonb_build_object('title_en', 'Hair Coloring', 'title_ar', 'صبغ الشعر', 'slug', 'hair-coloring', 'price', 300, 'duration', 90),
        jsonb_build_object('title_en', 'Facial', 'title_ar', 'عناية الوجه', 'slug', 'facial', 'price', 220, 'duration', 60),
    ...
    ELSIF v_type = 'clinic' THEN
      v_seed := jsonb_build_array(
        jsonb_build_object('title_en', 'Consultation', 'title_ar', 'استشارة', 'slug', 'consultation', 'price', 200, 'duration', 30),
        jsonb_build_object('title_en', 'Skin Treatment', 'title_ar', 'علاج البشرة', 'slug', 'skin-treatment', 'price', 350, 'duration', 60),
        jsonb_build_object('title_en', 'Follow-up Visit', 'title_ar', 'زيارة متابعة', 'slug', 'follow-up-visit', 'price', 120, 'duration', 30),
        jsonb_build_object('title_en', 'Care Session', 'title_ar', 'جلسة عناية', 'slug', 'care-session', 'price', 250, 'duration', 45)
      );
    ELSE
      v_seed := jsonb_build_array(
        jsonb_build_object('title_en', 'Service Session', 'title_ar', 'جلسة خدمة', 'slug', 'service-session', 'price', 180, 'duration', 45),
        jsonb_build_object('title_en', 'Consultation', 'title_ar', 'استشارة', 'slug', 'consultation', 'price', 150, 'duration', 30),
        jsonb_build_object('title_en', 'Care Package', 'title_ar', 'باقة عناية', 'slug', 'care-package', 'price', 300, 'duration', 90),
        jsonb_build_object('title_en', 'Follow-up', 'title_ar', 'متابعة', 'slug', 'follow-up', 'price', 120, 'duration', 30)
      );
    END IF;
