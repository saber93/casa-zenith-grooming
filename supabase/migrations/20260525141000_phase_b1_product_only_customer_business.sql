-- Phase B.1 correction: infer product-only checkout business from the customer profile before falling back to default business.
...
BEGIN
  ...
  IF v_business_id IS NULL AND v_customer_id IS NOT NULL THEN
    SELECT *
    INTO v_customer
